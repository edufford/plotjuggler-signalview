...
DataSetsPanel::DataSetsPanel(QWidget* parent)
    : QWidget(parent)
{
...
void DataSetsPanel::refresh(const OverlayManager* mgr)
{
  ...
  for (const auto& layer : mgr->layers())
  {
    ...
    rw.prefix_label->setStyleSheet("color: #000; font: bold 9pt monospace; min-width: 24px;");
    hl->addWidget(rw.prefix_label);

    // Display name (filename or "PJ Data")
    rw.name_label = new QLabel(QString::fromStdString(layer.display_name), row_widget);
    rw.name_label->setStyleSheet("color: #000; font: 9pt monospace;");
    rw.name_label->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
    hl->addWidget(rw.name_label);

    // Time offset
    QString offset_str = QString("offset: %1s")
        .arg(layer.time_offset, 0, 'f', 3);
...
void DataSetsPanel::contextMenuEvent(QContextMenuEvent* event)
{
  ...
  connect(style_action, &QAction::triggered, this, [this, clicked_layer]() {
    emit styleLayerRequested(clicked_layer);
  });
  ...
  connect(rename_action, &QAction::triggered, this, [this, clicked_layer]() {
    ...
    for (const auto& row : _rows)
    {
      ...
      {
        bool ok;
        QString new_name = QInputDialog::getText(this, "Rename Layer",
            "Display name:", QLineEdit::Normal, row.name_label->text(), &ok);
  ...
  connect(load_action, &QAction::triggered, this, [this]() {
    emit loadOverlayRequested();
  });
