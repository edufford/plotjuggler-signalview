...
void OverlayManager::setBaseData(PJ::PlotDataMapRef* data)
{
  ...
  // Always create a base layer entry (PJ data may be empty at init and fill later)
  if (data)
  {
    OverlayLayer base;
    base.index = 1;
...
std::optional<ResolvedSignal> OverlayManager::resolveSignal(const std::string& prefixed_name) const
{
...
std::vector<std::string> OverlayManager::findMatchingSignals(int layer_index,
...
OverlayManager::ParsedName OverlayManager::parsePrefixedName(const std::string& name)
{
  ParsedName result;
...
std::string OverlayManager::makePrefixedName(int layer, const std::string& raw_name)
{
...
std::unique_ptr<PJ::PlotDataMapRef> OverlayManager::parseCSV(const std::string& file_path)
{
  ...
  while (std::getline(file, line))
  {
    ...
    for (size_t i = 0; i < series_iters.size() && (i + 1) < values.size(); i++)
    {
      series_iters[i]->second.pushBack(PJ::PlotData::Point(time, values[i + 1]));
    }
  }
